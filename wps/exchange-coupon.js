...
function run(row) {
  ...
  const mangaPointApi = () => request(mangaPointUrl),
    exchangeMangaShopApi = (product_id = 1938, point = 100, product_num = 1) =>
      request(exchangeMangaShopUrl, {
        body: `product_id=${product_id}&point=${point}&product_num=${product_num}`,
      });
  ...
  function exchange(num) {
    try {
      ...
      // 库存不足，且时间是 xx:01 之前
      if (code === 2 && msg.includes('库存') && new Date().getMinutes() < 1) {
