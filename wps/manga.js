...
const clockInUrl = 'https://manga.bilibili.com/twirp/activity.v1.Activity/ClockIn?platform=android',
  shareComicUrl =
    'https://manga.bilibili.com/twirp/activity.v1.Activity/ShareComic?platform=android&channel=bilicomic&mobi_app=android_comic&is_teenager=0',
  seasonInfoUrl =
    'https://manga.bilibili.com/twirp/user.v1.SeasonV2/GetSeasonInfo?device=android&platform=android&channel=bilicomic&mobi_app=android_comic&is_teenager=0';
mangaDetailUrl =
  'https://manga.bilibili.com/twirp/comic.v1.Comic/ComicDetail?device=android&platform=android&version=4.16.0&mobi_app=android_comic&is_teenager=0';
...
function run(row) {
  ...
  function getNeedReadManga() {
    try {
      return seasonInfoApi()
        .json()
        .data.day_task.book_task.filter(task => task.user_read_min < 5);
    } catch (e) {
  ...
  function readManga() {
    ...
    for (const { user_read_min, read_min, id, title } of needReadManga) {
      ...
      try {
        const res = HTTP.get(
          `https://bo.js.cool/api/read_manga?mid=${mid}&comic=${id}&ep=${epid}&time=${read_min - user_read_min}`,
        );
